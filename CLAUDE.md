...
- `save_note` / `search_news` / `update_interests` / `set_push_enabled` / `set_push_interval` — LangChain tools via the `@tool` decorator (`langchain_core.tools`), not hand-written JSON schemas. `TOOLS` is `[save_note, search_news, update_interests, set_push_enabled, set_push_interval]`. The last three read/write `users_db.py`, keyed off `chat_id` injected via `ToolRuntime.context` (see `run_agent`'s `context` param below), not supplied by the model.
