...
# --- push_outcomes queries ------------------------------------------------


def _outcome(chat_id, outcome, at):
    users_db.record_push_outcome(chat_id, outcome, at)
...
def test_push_delivery_ratio_counts_only_cycles_that_called_the_model(isolated_subscribers_db):
    ...
    _outcome(1, users_db.PUSH_DELIVERED, now)
    _outcome(2, users_db.PUSH_CHAT_NOT_FOUND, now)
    _outcome(3, users_db.PUSH_CHAT_NOT_FOUND, now)
    _outcome(4, users_db.PUSH_NOTHING_NEW, now)
    _outcome(5, users_db.PUSH_NO_INTERESTS, now)

    delivered, generated = users_db.push_delivery_ratio(now - timedelta(hours=24))
...
def test_push_delivery_ratio_is_zero_zero_when_nothing_was_generated(isolated_subscribers_db):
    ...
    every idle night."""
    now = datetime(2026, 8, 21, 12, 0, tzinfo=timezone.utc)
    _outcome(1, users_db.PUSH_NOTHING_NEW, now)

    assert users_db.push_delivery_ratio(now - timedelta(hours=24)) == (0, 0)


def test_push_outcome_counts_respects_the_window(isolated_subscribers_db):
    now = datetime(2026, 8, 21, 12, 0, tzinfo=timezone.utc)
    _outcome(1, users_db.PUSH_DELIVERED, now - timedelta(hours=1))
    _outcome(1, users_db.PUSH_DELIVERED, now - timedelta(hours=48))

    assert users_db.push_outcome_counts(now - timedelta(hours=24)) == {users_db.PUSH_DELIVERED: 1}


def test_recent_outcomes_for_is_newest_first_and_per_subscriber(isolated_subscribers_db):
    now = datetime(2026, 8, 21, 12, 0, tzinfo=timezone.utc)
    _outcome(1, users_db.PUSH_DELIVERED, now - timedelta(hours=2))
    _outcome(1, users_db.PUSH_CHAT_NOT_FOUND, now - timedelta(hours=1))
    _outcome(2, users_db.PUSH_BLOCKED, now)

    assert users_db.recent_outcomes_for(1) == [
        users_db.PUSH_CHAT_NOT_FOUND,
        users_db.PUSH_DELIVERED,
    ]
    assert users_db.recent_outcomes_for(2) == [users_db.PUSH_BLOCKED]
...
def test_recent_outcomes_for_breaks_same_timestamp_ties_by_insertion_order(isolated_subscribers_db):
    ...
    now = datetime(2026, 8, 21, 12, 0, tzinfo=timezone.utc)
    _outcome(1, users_db.PUSH_DELIVERED, now)
    _outcome(1, users_db.PUSH_BLOCKED, now)

    assert users_db.recent_outcomes_for(1)[0] == users_db.PUSH_BLOCKED


def test_prune_push_outcomes_drops_only_rows_past_the_window(isolated_subscribers_db):
    now = datetime(2026, 8, 21, 12, 0, tzinfo=timezone.utc)
    _outcome(1, users_db.PUSH_DELIVERED, now - timedelta(days=200))
    _outcome(1, users_db.PUSH_DELIVERED, now - timedelta(days=1))

    assert users_db.prune_push_outcomes(now) == 1
    assert users_db.recent_outcomes_for(1) == [users_db.PUSH_DELIVERED]
...
def test_record_push_outcome_assumes_utc_for_a_naive_timestamp(isolated_subscribers_db):
    ...
    users_db.record_push_outcome(1, users_db.PUSH_DELIVERED, naive)

    aware = naive.replace(tzinfo=timezone.utc)
    assert users_db.push_outcome_counts(aware - timedelta(minutes=1)) == {users_db.PUSH_DELIVERED: 1}


def test_consecutive_chat_not_found_counts_a_run(isolated_subscribers_db):
    now = datetime(2026, 8, 21, 12, 0, tzinfo=timezone.utc)
    for i in range(3):
        _outcome(1, users_db.PUSH_CHAT_NOT_FOUND, now + timedelta(minutes=i))

    assert users_db.consecutive_chat_not_found(1) == 3
...
def test_consecutive_chat_not_found_stops_at_the_last_delivery(isolated_subscribers_db):
    ...
    now = datetime(2026, 8, 21, 12, 0, tzinfo=timezone.utc)
    _outcome(1, users_db.PUSH_CHAT_NOT_FOUND, now)
    _outcome(1, users_db.PUSH_DELIVERED, now + timedelta(minutes=1))
    _outcome(1, users_db.PUSH_CHAT_NOT_FOUND, now + timedelta(minutes=2))

    assert users_db.consecutive_chat_not_found(1) == 1
...
def test_consecutive_chat_not_found_skips_cycles_that_attempted_no_send(isolated_subscribers_db):
    ...
    now = datetime(2026, 8, 21, 12, 0, tzinfo=timezone.utc)
    _outcome(1, users_db.PUSH_CHAT_NOT_FOUND, now)
    _outcome(1, users_db.PUSH_NOTHING_NEW, now + timedelta(minutes=1))
    _outcome(1, users_db.PUSH_NOT_RELEVANT, now + timedelta(minutes=2))
    _outcome(1, users_db.PUSH_CHAT_NOT_FOUND, now + timedelta(minutes=3))

    assert users_db.consecutive_chat_not_found(1) == 2
...
def test_consecutive_chat_not_found_is_zero_for_a_healthy_subscriber(isolated_subscribers_db):
    ...
    assert users_db.consecutive_chat_not_found(1) == 0
