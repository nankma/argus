...
> **這個但書在 2026-08-19 於我們自己的資料中得到確認**，就在本調研寫完的
> 隔天——在真實快取上實測 TF-IDF 相似度時，分數最高的兩組「跨來源」配對
> 結果是 Hacker News 連到 BBC 文章的條目，帶著 BBC 一模一樣的標題。那是
> 同一篇文章被算了兩次，不是兩家媒體互相印證，而且它灌水的正好是那個本來
> 就已經過度代表的來源。快取中有 16 個標題因此重複出現。詳見
> `news-ranking-plan.md` 的「聚合站不等於印證」一節。抽象的但書變成了具體
> 的要求：**在計數任何東西之前，先把聚合站條目歸併回其原始出處。**
...
**1b.「多樣性」不是單一軸——必須指明你講的是哪一種。** 2026-08-19 實測後
補上。**內容多樣性**（這些項目講的是不同的事嗎？）與**來源多樣性**（這些
項目來自不同的地方嗎？）是兩個分開的目標，而且優化其中一個**不會**帶來
另一個。在本專案真實快取上實測：以內容相似度做 MMR 之後，來源集中度幾乎
毫無變化（有效數量 3.1 → 3.1），因為那個主導來源發布的是許多**確實不同**
的故事——MMR 根本找不到冗餘可以消除。同一份資料上，每來源上限達到有效
數量 11.1。詳見 `news-ranking-plan.md` 的「相似度無法解決來源塌陷」一節。
可推廣的重點：**下方第三節整個挑選階段家族，優化的都是你的相似度函數所
量測的東西——所以如果來源身分很重要，它就必須被放進那個函數裡**，而不是
假設它會自動從內容相異性中衍生出來。
